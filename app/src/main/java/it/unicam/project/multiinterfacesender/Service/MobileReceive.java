...
public class MobileReceive extends Service {
    ...
    public IBinder onBind(Intent intent) {
        return new IService_App_to_Mobile.Stub() {
            ...
            public void register(IService_Mobile_to_App activity) throws RemoteException {
                MobileReceive.this.iService_mobile_to_app = activity;
                MobileReceive.this.iService_mobile_to_app.getProcessID(android.os.Process.myPid());
            }
    ...
    private void connectionCreated() {
        final Handler handler = new Handler(MobileReceive.this.getMainLooper());
        handler.post(() -> {
            try {
                MobileReceive.this.iService_mobile_to_app.mobileHandler(MOBILE_STATE_CONNECTED);
    ...
    private void connectionEstablished() {
        ...
        handler.post(() -> {
            try {
                MobileReceive.this.iService_mobile_to_app.mobileHandler(MOBILE_STATE_ESTABILISHED);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        });
    ...
    private void connectionRefused() {
        ...
        handler.post(() -> {
            try {
                MobileReceive.this.iService_mobile_to_app.mobileHandler(MOBILE_STATE_NOT_FOUND);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        });
    ...
    private void connectionClosed() {
        ...
        handler.post(() -> {
            try {
                MobileReceive.this.iService_mobile_to_app.mobileHandler(MOBILE_STATE_CLOSED);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        });
